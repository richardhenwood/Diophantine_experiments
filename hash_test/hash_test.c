...
static value_pair *
new_value_pair () {
    //return  malloc (sizeof(value_pair));
    return g_new0 (value_pair, 1);
}

static void 
free_pair (value_pair *vp) {
    //g_free(vp->u); 
    //g_free(vp->v); 
    //g_free(vp->uvstr);
    g_free(vp); 
...
//destroy_hash_values_func (gpointer data) {
static void
destroy_hash_values_func (GSList *list) {
    g_slist_foreach(list, (GFunc)free_pair, NULL);
    g_slist_free(list);
...
//    GSList *iter = list;
//    while (iter) {
////        struct value_pair * uv_pair = (struct value_pair *)iter->data;
//
//        //printf("destroy value = (%lld, %lld)\n", uv_pair->u, uv_pair->v);
//
////        g_free(iter->data);
//        //g_slice_free(struct value_pair, uv_pair);
//        g_slice_free(value_pair, iter->data);
//        iter = iter->next;
//    }
//
//    g_slist_free(iter);
...
//    GSList *iter = NULL;
    ...
    //value_pair * uv_pair = g_new(struct value_pair, 1);
    value_pair * uv_pair = new_value_pair();
...
//    struct value_pair *uv_pair = g_slice_new(struct value_pair);
    ...
    while (current_u <= height) {
        ...
        while (current_v <= height) {
            ...
            uv_pair = g_new(value_pair, 1);
            //uv_pair = malloc(sizeof(value_pair));
            uv_pair->u = current_u;
            uv_pair->v = current_v;
            //sprintf(uv_pair->uvstr, "%lld,%lld", current_u, current_v);
            //uv_pair->uvstr = "hellowork";
